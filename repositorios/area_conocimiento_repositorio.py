"""
area_conocimiento_repositorio.py — Repositorio para area_conocimiento.
Implementa operaciones CRUD para PostgreSQL.
"""
from repositorios.base_repositorio import BaseRepositorioPostgreSQL
from typing import Any


class RepositorioAreaConocimientoPostgreSQL(BaseRepositorioPostgreSQL):
    """Repositorio concreto para área de conocimiento en PostgreSQL."""

    def __init__(self, cadena_conexion: str):
        super().__init__(cadena_conexion)
        self._tabla = "area_conocimiento"
        self._pk = "id"
    ...
    async def crear(self, datos: dict, esquema: str = "public") -> bool:
        query = f"""
            INSERT INTO {esquema}.{self._tabla} (id, gran_area, area, disciplina)
    ...
    async def eliminar(self, id_valor: Any, esquema: str = "public") -> bool:
        # Borrado lógico: marcar como inactivo (si existe campo activo)
        # Para esta tabla usamos borrado físico por no tener campo activo
